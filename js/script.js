...
function toggleMusic() {
  ...
  if (audio.paused) {
    ...
    if (btn) btn.style.animation = "spin 4s linear infinite";
  } else {
    audio.pause();
    if (icon) icon.innerText = "🔇";
...
// ===== REPLY =====
function setReply(id, name) {
  const replyID = document.getElementById("form-replyID");
  ...
  if (replyName) replyName.innerText = "@" + name;
  if (pesan) {
    pesan.value = "@" + name + " ";
    pesan.focus();
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll("button")
    .forEach((b) => b.addEventListener("click", haptic));

  // ===== GUEST NAME =====
  const params = new URLSearchParams(window.location.search);
  const tamu = decodeURIComponent(params.get("to") || "Tamu Undangan").replace(
    /\+/g,
    " "
  );
  const guestDisplay = document.getElementById("guest-display");
  const formNama = document.getElementById("form-nama");
  if (guestDisplay) guestDisplay.innerText = tamu;
  if (formNama) formNama.value = tamu;
  ...
  // ===== COUNTDOWN =====
  const target = new Date("2025-12-31T08:00:00");
